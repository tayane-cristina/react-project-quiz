...
const Science = () => {
  ...
  const scienceQuestions = Data.filter(question => question.category === "science");
  ...
  const btnOptionsInitialState = () => {
    ...
    btnOne.style.backgroundColor = 'rgb(39, 116, 16)'
    btnTwo.style.backgroundColor = 'rgb(39, 116, 16)'
    btnThree.style.backgroundColor = 'rgb(39, 116, 16)'
    btnFour.style.backgroundColor = 'rgb(39, 116, 16)'
  }
  ...
  const callNextQuestion = () => {
    ...
    if (nextQuestion < scienceQuestions.length) {
  ...
  return (
    <div className='div-principal-science'>
      <div className='category-title title-science'></div>
      {showScore ? (
        <div className='score-section'>
          <p>Você acertou {score} de {scienceQuestions.length} perguntas!</p>
          <p>{resultMessage}</p>
        </div>
      ) : (
        <>
          <div className='question-section'>
            <div className='question-count'>
              <span>Pergunta {currentQuestion + 1}</span>/{scienceQuestions.length}
            </div>
            <h3 className='question-text'>{scienceQuestions[currentQuestion].questionText}</h3>
          </div>
          <div className='answer-section'>
            {scienceQuestions[currentQuestion].answerOptions.map((answerOption, index) => (
              <button className='answer-option-btn science' key={index} onClick={(e) => handleAnswerOptionClick(e, answerOption.isCorrect)}>
